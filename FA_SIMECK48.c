...
int matchPattern(uint32_t data);
uint8_t getFaultPosition(uint32_t delta);
int satisfatory(uint8_t array[]);
int satisfatoryMaster(uint8_t *array);
int rightPosition(int pos);
void delay(int number_of_seconds);
void getK1(uint32_t cipher[2], uint32_t constant, uint32_t seq, uint32_t keys[4]);
void getMasterkey(uint32_t cipher[2], uint32_t constant, uint32_t seq, uint32_t keys[4]);

const int NUM_ROUNDS = 36;    
const int8_t WORD_SIZE = 24;    
...
int main(void) {
    ...
    const uint32_t *master_key = key; 
    const uint32_t *plaintext = text; 
...
void getMasterkey(uint32_t cipher[2], uint32_t constant, uint32_t seq, uint32_t keys[4]) {
    ...
    uint32_t correctcipher[2];              // for encryption correct text
    uint32_t faultycipher[2];               // for encryption faulty text
    ...
    uint32_t tempkeys[4]; 
    uint32_t correcttext[6]; //uint16_t correcttext[6][2]; // imm     
	uint32_t faultytext[6]; //uint16_t faultytext[6][2];
    uint32_t delta[6];   //uint16_t delta[6][2];    
    uint32_t temp;
    int temppos;
    ...
    {           
        ...
        while (!satisfatoryMaster((uint8_t *)stateKeys))
        {
            n_faults +=1;
...
void getK1(uint32_t cipher[2], uint32_t constant, uint32_t seq, uint32_t keys[4]) {
    ...
    uint32_t correcttext[6];    // Correct left inputs from fault 
	uint32_t faultytext[6];     // Faulty left inputs from fault
    uint32_t delta[6];          // Input differences from fault 
    uint32_t tempkeys[4]; 
    uint32_t temp;
    int temppos;
    int8_t pos;                 // Position where a fault is injected
    ...
    {   
        ...
        while (!satisfatory(state)) //&& (n_faults) < 100)
        {
            n_faults +=1;
